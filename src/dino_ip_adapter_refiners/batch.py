...
class TypeCheckMeta(type):
    def __new__(cls, name: str, bases: tuple[type, ...], dct: dict[str, Any]) -> type:
        ...
        if len(attr_types) == 0:
            raise ValueError(
                f"At least one attribute is required for '{new_class.__name__}'"
            )

        for attr_name, attr_type in attr_types.items():
            if attr_type not in [Tensor, list]:
                raise TypeError(
                    f"Type of '{attr_name}' must be 'Tensor' or 'list', got '{attr_type.__name__}'"
                )
...
class BaseBatch(metaclass=TypeCheckMeta):
    ...
    def __init__(self, **kwargs: AttrType):
        ...
        for attr_name in kwargs:
            ...
            if size is not None and size != new_size:
                raise ValueError(
                    f"Attribute '{attr_name}' has size {new_size}, expected {size}"
                )
            size = new_size

            if size == 0:
                raise ValueError(
                    f"Attribute '{attr_name}' is empty, empty attributes are not permitted"
                )
    ...
    def __setattr__(self, name: str, value: Any, check_size: bool = True) -> None:
        ...
        if name in attr_types:
            ...
            if check_size and new_size != len(self):
                raise ValueError(
                    f"Attribute '{name}' has size {new_size}, expected {len(self)}"
                )
    ...
    def collate(cls: Type[T], batch_list: list[T]) -> T:
        ...
        for attr_name, attr_type in attr_types.items():
            ...
            if attr_type == Tensor:
                ...
                collated_attrs[attr_name] = [
                    item for sublist in attr_list for item in sublist
                ]
    ...
    def __add__(self: T, other: Any) -> T:
        ...
        for attr_name, attr_type in attr_types.items():
            ...
            if attr_type == Tensor:
                collated_attrs[attr_name] = cat(
                    tensors=(self_attr, getattr(other, attr_name)), dim=0
                )
            else:
    ...
    def split(self: T) -> list[T]:
        ...
        for i in range(len(self)):
            args = {
                attr_name: getattr(self, attr_name)[i : i + 1]
                for attr_name in self.__class__.attr_types()
            }
        ...
        match key:
            case key if isinstance(key, slice):
                return self.__class__(
                    **{
                        attr_name: getattr(self, attr_name)[key]
                        for attr_name in self.__class__.attr_types()
                    }
                )
